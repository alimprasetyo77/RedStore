    ...
    <Layout>
      <div className="flex min-h-screen ">
        ...
        <div className="w-4/12 p-16 ">
          <form onSubmit={handleSubmit(handleRegister)} className="flex flex-col items-start  gap-y-10 pt-16">
            <h3 className="text-3xl font-semibold">Create an account</h3>
            <div className="p-4 flex flex-col w-full gap-y-8">
              <input type="text" placeholder="Name" className="border-b px-5 py-1 outline-none" {...register("name")} disabled={isSubmitting} aria-disabled={isSubmitting} />
              {errors.name && <p className="text-sm text-red-500 -mt-3">{errors.name.message}</p>}

              <input type="text" placeholder="Username" className="border-b px-5 py-1 outline-none" {...register("user_name")} disabled={isSubmitting} aria-disabled={isSubmitting} />
              {errors.user_name && <p className="text-sm text-red-500 -mt-3">{errors.user_name.message}</p>}
            ...
            <div className="flex flex-col w-full gap-y-3">
              <button className="w-full py-3 rounded-md bg-red-500 text-white " disabled={isSubmitting} aria-disabled={isSubmitting}>
                {isSubmitting ? (
