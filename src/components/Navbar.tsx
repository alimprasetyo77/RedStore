...
const Navbar = () => {
  ...
  return (
    <div className="w-full shadow py-4 bg-red-500 text-white sticky top-0 z-50">
      <div className="container flex items-center mx-auto justify-between md:justify-start gap-3 ">
        <Link to={"/"}>
          <img src={brand} alt="logo-brand" width={80} height={80} />
        </Link>
        <ul className="md:flex items-center flex-grow justify-center gap-x-8 hidden ">
          <Link to={"/"} className={`${location.pathname === "/" && "font-semibold"} cursor-pointer`}>
            <li>Home</li>
          </Link>
          {user.role === "admin" ? (
            <>
        ...
        {token && user.role !== "admin" ? (
          <div className="flex items-center gap-x-3 md:gap-x-7 md:ml-6 ">
            <Link to={"/cart"} className="relative cursor-pointer hover:bg-red-400/70 p-1 rounded-full duration-300">
              <ShoppingCart />
              <span className="absolute z-10 -top-2 -right-2 bg-white font-medium text-black size-4 flex items-center justify-center text-xs rounded-full p-2">{carts ? carts.length : 0}</span>
            </Link>
            <DropdownMenu>
